...
<body>
        ...
        {else}
            ...
            <div class="row invoice-header">
                <div class="invoice-col">
                    ...
                    {if $logo}
                        <p><img src="/logo.png" title="{$companyname}" /></p>
                    {else}
                        <h2>{$companyname}</h2>
                    {/if}
                    <h3>{$pagetitle}</h3>
            ...
            <div class="row">
                <div class="invoice-col right">
                    ...
                    <div id="paymentMethodWarning" class="alert alert-warning" style="display: none;">
                        ⚠️Using PayPal or Stripe will enable auto renewal until cancelled manually!<br>
                        ⚠️使用 PayPal 或 Stripe 付款将启用自动续订，需手动取消！<br>
                        ⚠️PayPal または Stripe を使用すると、自動更新が有効になります。手動でキャンセルするまで継続されます！
                    </div>
                    <span class="small-text" data-role="paymethod-info">
                        {if $status eq "Unpaid" && $allowchangegateway}
                            <form method="post" action="{$smarty.server.PHP_SELF}?id={$invoiceid}" class="form-inline" id="gatewayForm">
                                {$gatewaydropdown}
                            </form>
                        {else}
    ...
    <p class="text-center hidden-print"><a href="clientarea.php?action=invoices">{$LANG.invoicesbacktoclientarea}</a></p>
    ...
    <!-- Payment method warning script -->
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        const gatewayDropdown = document.querySelector('select[name="gateway"]');
        const gatewayForm = document.getElementById('gatewayForm');
        const warningDiv = document.getElementById('paymentMethodWarning');
        const warningMessage = warningDiv.innerText;

        // Show warning immediately if Stripe or PayPal is selected
        if (gatewayDropdown) {
            const initialGateway = gatewayDropdown.value.toLowerCase();
            if (initialGateway === 'stripe' || initialGateway === 'paypalcheckout') {
                warningDiv.style.display = 'block';
            }

            // Store the initial value
            gatewayDropdown.setAttribute('data-default', gatewayDropdown.value);

            gatewayDropdown.addEventListener('change', function() {
                const selectedGateway = this.value.toLowerCase();
                
                // Check if the selected gateway is Stripe or PayPal
                if (selectedGateway === 'stripe' || selectedGateway === 'paypalcheckout') {
                    warningDiv.style.display = 'block';
                    const confirmChange = confirm(warningMessage);
                    ...
                    if (confirmChange) {
                        ...
                        warningDiv.style.display = selectedGateway === 'stripe' || selectedGateway === 'paypalcheckout' ? 'block' : 'none';
                    }
                } else {
